...
acf(
    ...
    lag.max = 36,
    main = "Residuals ACF plot",
    sub = "HOUSTNSA ~ c + Jan + Feb + Mar + Apr + May + Jun + Jul + Aug + Sep + Oct + Nov"
)
```

<!-- Analysis -->
The residual ACF plot for the linear model HOUSTNSA ~ c + Jan + Feb + Mar + Apr + May + Jun + Jul + Aug + Sep + Oct + Nov, shows a significant difference compared to the residual ACF plot for the linear model HOUSTNSA ~ 1. 
The linear model HOUSTNSA ~ c + Jan + Feb + Mar + Apr + May + Jun + Jul + Aug + Sep + Oct + Nov, has no seasonality in the residual acf plot since the linear model has already captured it. What is remaining is the trend as depicted by decreasing values as the lags increase. Thus showing that the seasonal dummy variables have improved the model. 


## 2(d) (9 marks)

```{r, echo=FALSE}
AR_model <- arima(db$HOUSTNSA, order = c(2, 0, 0), xreg = db[3:13])
# AR_model
# ar()
```
...
DF1_denom <- df.residual(fit1.UR) # degrees of freedom of UR linear model = 746
