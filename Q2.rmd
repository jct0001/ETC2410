...
\begin{align*}
    \hat{u}_i &= residuals \\
    {\hat{u}}_i &= y_i-{\hat{y}}_i,\ i = 1, 2, ... n && \text {from Assignment eq(7)}\\
...
\begin{align*}
    ...
    &= n \bar y - n \hat \beta_i \bar{x} - n (\bar y - \hat \beta_i \bar{x}) && \text {substituting from Assignment eq(6)} \\
...
\begin{align*}
    ...
    \left.\frac{\partial SSR\left(b_0,b_1\right)}{\partial b_0}\right|_{{\hat{\beta}}_0,{\hat{\beta}}_1} &= -2\sum_{i=1}^{n}{\left(y_i-\beta_0-\beta_1x_i\right)=0} \\
    \left.\frac{\partial SSR\left(b_0,b_1\right)}{\partial b_i}\right|_{{\hat{\beta}}_0,{\hat{\beta}}_1} &= -2\sum_{i=1}^{n}{x_i\left(y_i-b_0-b_1x_i\right)=0} && \text {(1)} \\
    \\
    {\hat{u}}_i &= y_i-{\hat{y}}_i,\ i = 1, 2, ... n && \text {from Assignment eq(7)}\\
    &= y_i-\left({\hat{\beta}}_o+{\hat{\beta}}_1x_i\right) && \text {by definition } \hat y_i = \hat \beta_0 + \hat \beta_1 x_i\\
    &= y_i-{\hat{\beta}}_o-{\hat{\beta}}_1x_i && \text {(2)}\\
\end{align*}

\begin{align*}
    -2\sum_{i=1}^{n}x_i (\hat u_i) &= 0 && \text {substituting (2) into (1)} \\
    \sum_{i=1}^{n}x_i \hat u_i &= 0 && \text {(3)}
\end{align*}
...
\begin{pmatrix}
    ...
    x_{n}
\end{pmatrix}}$ and 
$\boldsymbol{\hat u} =\underset{(u \times 1)}{ 
\begin{pmatrix}
    \hat u_{1} \\
    \vdots \\
    \hat u_{n}
...
\begin{align*}
    {\boldsymbol{x' \hat u}} &= \begin{pmatrix} \hat x_{1} && ... && \hat x_{n} \end{pmatrix}
