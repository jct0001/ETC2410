...
```
<!-- $$`r glue::glue("Hi, {alpha_1}")`$$ -->

\begin{align*}
    H_0 &: \beta_1 = 0 \\
    H_1 &: \beta_1 > 0 \\
    \text {Significant Level} &: \alpha = 0.05 \\
    \text {Test statistic and null distribution} &: \frac{\hat \beta_1}{se(\hat \beta_1)} \sim t_{n-k-1} = t_{472} \\
...
\begin{align*}
    ...
    \text {Significant Level} &: \alpha = 0.05 \\
    \text {Test statistic and null distribution} &: \frac{(SSR_R - SSR_{UR})}{SSR_{UR}} \frac{(n-k-1)}{q} \sim F_{(q, n-k-1)} = t_{5, 468} \\
    t_{calc} &= `r tstat2` \\
    F_{crit} &= `r fcrit2` \\
...
\begin{align*}
    ...
    \text {Test statistic and null distribution} &: \frac{(SSR_R - SSR_{UR})}{SSR_{UR}} \frac{(n-k-1)}{q} \sim F(q, n-k-1) = F(2, 468) \\
    t_{calc} &= `r tstat3` \\
    F_{crit} &= `r fcrit3` \\
    \text {Decision rule} &: \text {reject $H_0$ if $t_{calc} > F_{crit}$} \\
...
$$
\widehat{logsal} = \underset{(0.164)}{8.282} - \underset{(0.020)}{0.120}U + 
...
The null hypothesis is that, controlling for education, starting salary, and job category, the racial pay gap for males and females is the same. Hence, from part (h)ii) and part (h)iii) this statement is equivalent to the stating that $\beta_{4} + \beta{6} = \beta_{4}$ which in other words is to say that $\beta_{6} = 0$.
...
\begin{align*}
    ...
    H_1 &: \beta_1 \neq 0 \\
    \text {Significant Level} &: \alpha = 0.1 \\
    \text {Test statistic and null distribution} &: \frac{\hat \beta_1}{se(\hat \beta_1)} \sim t_{n-k-1} = t_{467} \\
