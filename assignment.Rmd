...
We conclude at the 0.05 level of significance that race has no effect on (the log of) an individual's annual salary in favour of the alternative hypothesis race has an effect.
...
$\beta_{0}$ measures the conditional mean of the log of an individual's annual salary in this bank for one who does not belong to an ethnic minority. This value is 10.396. Hence, this means that the conditional mean of an individual's annual salary for one who does not belong to an ethnic minority is $e^{10.396}$.
...
(ix) Since $\beta_{0} = 0$, in a hypothesis test of individual significance at  0.05, does not fall within the 95 per cent confidence interval ${10.356, 10.436}$ we conclude that $\hat{\beta_{0}}$ is statistically significant.
...
So, according to our model only gender and race are individually insignificant at this level as $\textrm{p-value}_{gender} = 0.0013 > 0.001$ and $\textrm{p-value}_{race} = 0.027 > 0.001$
...
We conclude at the 0.05 level of significance that an individual's number of years of education, their gender, the job category within the bank, their log of their starting annual salary, and if they belong to an ethnic minority or not are jointly insignificant in effecting (the log of an) individual's annual salary in favour of the alternative hypothesis that at least one of these variables are significant.
...
We conclude at the 0.05 level of significance that an individual in the bank's gender and whether or not the belong to ethnic minority has no effect on the (log of their) annual salary in favour of the alternative hypothesis that their gender and / or their race does effect the individual's (log of their) annual salary.
...
This expression is our restricted model. We will use these two models for our F-test when we come to test the null hypothesis that $\beta_{1} + \beta_{4} = \beta_{3}$ against the alternative hypothesis that $\beta_{1} + \beta_{4} \neq \beta_{3}$, that the two populations A and B do not have the same average `logsal`.
...
Given $\mathbb{E}[logsal \mid educ, logssal, gender, race, jobcat] = \beta_{0} + \beta_{1} \ educ + \beta_{2} \ logssal + \beta_{3} \ gender + \beta_{4} \ race + \beta_{5} \ jobcat$, it follows that:
...
The racial pay grap for males, controlling for education, starting salary, and job cateogry is:
$$
\mathbb{E}[logsal \mid gender = 1, race =1, educ, logssal, jobcat] - \mathbb{E}[logsal \mid gender = 1, race = 0, educ, logssal, jobcat]
$$
...
The racial pay grap for females, controlling for education, starting salary, and job cateogry is:
$$
\mathbb{E}[logsal \mid gender = 0, race =1, educ, logssal, jobcat] - \mathbb{E}[logsal \mid gender = 0, race = 0, educ, logssal, jobcat]
$$
According to our new model, this is
...
The null hypothesis is that, controlling for education, starting salary, and job category, the racial pay gap for males and females is the same. Hence, from part ii. and part iii. this statement is equivalent to the stating that $\beta_{4} + \beta{6} = \beta_{4}$ which in other words is to say that $\beta_{6} = 0$,
...
abs(tstat5) > tcrit5
```
In conclusion, at 0.10 level of significance, we reject the null hypothes that, controlling for education, starting salary, and job category, the racial pay gap for males and females is the same in favour of the alternative hypothesis that they are not.
